...
const MARKETS_QUERY = `
  query GetMarkets($first: Int!, $skip: Int!, $where: FixedProductMarketMaker_filter) {
    fixedProductMarketMakers(first: $first, skip: $skip, where: $where, orderBy: creationTimestamp, orderDirection: desc) {
      id
      title
      question {
        id
...
const MARKET_UPDATES_QUERY = `
  query GetMarketUpdates($timestamp: BigInt!) {
    fixedProductMarketMakers(where: { lastActiveDay_gt: $timestamp }, orderBy: lastActiveDay, orderDirection: asc) {
      id
      title
      question {
        id
...
export class OmenSource implements DataSource {
  readonly name = MarketSource.OMEN;
  readonly isActive: boolean;
  
  private client: GraphQLClient;
  private pollInterval?: NodeJS.Timeout;
  private readonly logger = createSourceLogger('omen');
  ...
  async initialize(): Promise<void> {
    if (!this.isActive) {
      this.logger.info('Omen source is disabled or missing GraphQL endpoint');
      return;
    }

    this.logger.info('Initializing Omen source', {
    ...
    try {
      ...
      this.logger.info('Omen GraphQL connection successful');
    } catch (error) {
      throw new DataSourceError(
        'Failed to connect to Omen GraphQL endpoint',
        this.name,
  ...
  async startPolling(): Promise<void> {
    ...
    this.logger.info('Starting Omen polling', {
  ...
  async stopPolling(): Promise<void> {
    ...
    this.logger.info('Stopped Omen polling');
  }
  ...
  async subscribeToUpdates(callback: (event: MarketEventData) => void): Promise<void> {
    ...
    this.logger.info('Subscribed to Omen updates via polling');
  }

  async unsubscribeFromUpdates(): Promise<void> {
    this.updateCallback = undefined;
    this.logger.info('Unsubscribed from Omen updates');
  }
  ...
  private async testConnection(): Promise<void> {
    ...
    if (!response.fixedProductMarketMakers) {
  ...
  private async pollMarkets(): Promise<void> {
    try {
      ...
      const markets = response.fixedProductMarketMakers || [];
